$(function(){
  ...
  // ===================================
    function new_comment(comment_data){
      ...
      var HTML_endDiv =
        ...
        `
      if (comment_data.game_user.id == comment_data.user_id){
          // 出品者とコメントしたユーザーが等しい場合
        var html = HTML_content_time + HTML_deleteBtn + HTML_name + HTML_sellerMark + HTML_endDiv
      }else{
         // 出品者とコメントしたユーザーが異なる場合
        var html = HTML_content_time + HTML_name + HTML_endDiv
            };
  ...
  $(".comment_list").on('click',".comment_restore",function(e){
    ...
    .done(function(comment_data){
      if (comment_data.game_user.id == comment_data.user_id){   // 出品者とコメントユーザーが同じ場合
        var html = new_comment(comment_data);
